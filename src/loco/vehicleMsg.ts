...
export class MsgVehicleMode extends Message
{
	...
	constructor(header: Header, mode?: number[] | {opMode: OpMode, speedSteps: StepMax})
	{
		super(header);
		if(!mode) return;
		...
		if(Array.isArray(mode)) for(let byte of mode.slice(0, 2)) {
			...
			super.push({value: ((mode.opMode as number)<<4) | (mode.speedSteps), length: 1});
	...
	get stepMax(): StepMax | undefined {
		return this.data.length < 2 ? undefined : ((this.data[0].value as number) & 0xf) as StepMax}
	get opMode(): OpMode | undefined {
		return this.data.length < 2 ? undefined : ((this.data[0].value as number) >> 4) as OpMode}
	static fromBuffer(mode: MsgMode, buffy: Buffer) {
